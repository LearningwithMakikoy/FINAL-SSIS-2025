document.addEventListener('DOMContentLoaded', function() {
  ...
  let filteredColleges = colleges.slice();
  let editIndex = null;
  ...
  function renderTable(list = filteredColleges) {
    if (!tbody) return;
    ...
    pageData.forEach((c, i) => {
      const originalIndex = filteredColleges.findIndex(cl => cl.code === c.code);
  ...
  const tableHeaders = document.querySelectorAll('#colleges-table-body').closest('table')?.querySelectorAll('thead th');
  if (tableHeaders) {
    ...
    if (action === 'delete') {
      ...
      fetch(`/user/colleges/delete/${college.code}`, {
        ...
        if (data && data.success) {
          ...
          const filteredIndex = filteredColleges.findIndex(c => c.code === college.code);
          if (filteredIndex !== -1) {
            filteredColleges.splice(filteredIndex, 1);
          }
